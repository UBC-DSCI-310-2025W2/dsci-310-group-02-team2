...
The initial model achieved an overall accuracy of approximately `{python} f"{accuracy:.3f}"` on the test dataset. The confusion matrix shows that the model correctly classified a large majority of non-purchasing visitors (`{python} tn` true negatives) while making relatively few false positive predictions (`{python} fp`). This suggests that the model is effective at identifying users who are unlikely to complete a purchase.

However, the model performs less effectively when identifying actual purchasers. Out of `{python} tp + fn` visitors who made a purchase, only `{python} tp` were correctly classified, while `{python} fn` were misclassified as non-purchasers. This corresponds to a relatively low recall for the purchase class. In practical terms, the model tends to be conservative and predicts “Not Purchased” more frequently, which likely reflects the imbalance in the dataset where non-purchasing visitors are much more common.
